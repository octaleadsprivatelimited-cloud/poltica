...
export default function ManifestoPage({ params }: { params: { village: string; username: string } }) {
  ...
  useEffect(() => {
    loadManifesto();
  }, [params.village, params.username]);

  const loadManifesto = async () => {
    try {
      setLoading(true);
      const slug = `${params.village}/${params.username}`;
      const response = await fetch(`/api/manifesto/${slug}`);
